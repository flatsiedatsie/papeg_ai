...
console.log("TTS WORKER2 EXISTS");
...
class MyTextToSpeechPipeline {
    ...
    static async getSpeakerEmbeddings(speaker_id) {
		...
		//console.log("tts_worker2: speaker_embeddings_url: ", speaker_embeddings_url);
...
self.addEventListener('message', async (event) => {
	...
	//console.log("TTS event.data: ", event.data);
	
	if(typeof event.data.cache_name == 'string'){
		//console.log("tts_worker: setting cache_name to: ", event.data.cache_name)
	...
	if(typeof event.data.task == 'object'){
		...
		if(typeof event.data.task.sentence == 'string' && event.data.task.sentence.length){
			//console.log("TTS_WORKER: task has simple sentence property, using that as the input sentence: ", event.data.task.sentence);
			sentence = event.data.task.sentence; // simpler shortcut option for simple tasks
			
			//console.log("\n\nTOOK start, sentence: ", sentence);
			
			const start_stamp = Date.now();
			
		    // Load the pipeline
		    const [tokenizer, model, vocoder] = await MyTextToSpeechPipeline.getInstance(x => {
				//console.log("tts_worker2: posting download progress message: ", x);
			...
			const { input_ids } = tokenizer(sentence);
			
			//console.log('Took +tokenizer: ', (Date.now() - start_stamp) / 1000, 'ms using ', env.backends.onnx.wasm.numThreads, 'threads');
			...
			//console.log('Took +embeddings: ', (Date.now() - start_stamp) / 1000, 'ms using ', env.backends.onnx.wasm.numThreads, 'threads');
			...
			//console.log('Took +wav-encode', (Date.now() - start_stamp) / 1000);
