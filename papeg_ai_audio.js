...
function get_browser_tts_languages() {
	if(browser_synth){
		...
		//console.log("in get_browser_tts_languages. window.browser_tts_voices_raw: ", window.browser_tts_voices_raw);
	...
	if(typeof EasySpeech == 'undefined' && window.added_scripts.indexOf('./easy_speech/EasySpeech.iife.js') == -1){
		...
		.then((value) => {
			if(EasySpeech){
    			...
    			.catch(e => console.error('easy_speech init failed: ', e));
			}
